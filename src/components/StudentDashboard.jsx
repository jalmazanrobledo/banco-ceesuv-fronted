...
export default function StudentDashboard() {
  ...
  // Función para generar y descargar el Estado de Cuenta en PDF
  const descargarPDFEstadoCuenta = async () => {
    const input = document.getElementById('estado-de-cuenta-pdf');
    if (!input) {
      mostrarToast("No se encontró el formato del estado de cuenta.", "error");
      return;
    }
    ...
    try {
      ...
      pdf.save(`Estado_de_Cuenta_${nombreCompletoAlumno.replace(/\s+/g, '_')}_${nombreMesActual}_${anioSeleccionado}.pdf`);
      
      mostrarToast("¡Estado de cuenta descargado con éxito!", "success");
    } catch (error) {
      console.error("Error al generar el PDF:", error);
      mostrarToast("Hubo un error al generar el PDF.", "error");
    }
  };
